...
export function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  const [activeAccountTab, setActiveAccountTab] = useState<'referrals' | 'assets'>('assets');
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="bg-[#242427]/80 backdrop-blur-md p-1 rounded-xl flex border border-white/20 shadow-sm">
                        <button 
                          onClick={() => setActiveAccountTab('referrals')}
                          className={`flex-1 text-[13px] font-medium py-1.5 rounded-lg flex items-center justify-center gap-2 transition-all duration-300
                            ${activeAccountTab === 'referrals' ? 'bg-[#13151A] text-white shadow-sm border border-white/10' : 'text-white/50 hover:text-white'}`}
                        >
                        ...
                        <button 
                          onClick={() => setActiveAccountTab('assets')}
                          className={`flex-1 text-[13px] font-medium py-1.5 rounded-lg flex items-center justify-center gap-2 transition-all duration-300
                            ${activeAccountTab === 'assets' ? 'bg-[#13151A] text-white shadow-sm border border-white/10' : 'text-white/50 hover:text-white'}`}
                        >
                          My asset ({assets.length})
                        </button>
                      </div>

                      <AnimatePresence mode="wait">
                        {activeAccountTab === 'assets' ? (
                          <motion.div 
                            key="assets"
                          ...
                          >
                            {/* USER ASSETS LIST - Head of Designer Polish */}
                            <div className="flex-1 overflow-y-auto max-h-[320px] pr-1 space-y-2 scrollbar-hide mt-2">
                              {assets.length > 0 ? (
                                assets.map((asset) => (
                                  <div key={asset.id} className="bg-[#13151A]/80 border border-white/5 p-3 rounded-xl flex items-center justify-between group/asset hover:border-[#1863E1]/30 transition-all">
                                    <div className="flex items-center gap-3">
                                      <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#1863E1]/20 to-[#1863E1]/5 border border-[#1863E1]/20 flex items-center justify-center">
                                        <Zap className="w-5 h-5 text-[#1863E1]" />
                                      </div>
                                      <div className="flex flex-col">
                                        <span className="text-white text-sm font-black tracking-tight">{asset.symbol}</span>
                                        <span className="text-[10px] text-white/40 font-bold uppercase tracking-wider">{asset.source}</span>
                                      </div>
                                    </div>
                                    <div className="flex flex-col items-end">
                                      <span className="text-white text-sm font-black italic">+{asset.amount}</span>
                                      <span className="text-[#22c55e] text-[11px] font-bold">₹{asset.valueINR.toLocaleString()}</span>
                                    </div>
                                  </div>
                                ))
                              ) : (
                                <div className="flex flex-col items-center justify-center py-12 opacity-20">
                                   ...
                                   <p className="text-sm font-bold uppercase tracking-widest">No assets secured yet</p>
                                </div>
                              )}
                            </div>
                          </motion.div>
